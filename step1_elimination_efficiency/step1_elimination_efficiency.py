...
try:
    for index, row in final_df[2628:3000].iterrows():
        free_text = row['text']
...
llm_2628_3000_step1 = pd.DataFrame({'index': index_list, 'llm_2628_3000_step1': llm_response_list})

with open('llm_2628_3000_step1.pkl', 'wb') as file:
    pickle.dump(llm_2628_3000_step1, file)
...
try:
    for index, row in final_df[3000:3500].iterrows():
        free_text = row['text']
        user_message = step1_query_optimized.format(free_text=free_text)
        openai_message = create_prompt(system_message, user_message)
        response = send_message(openai_message, deployment_name)
...
llm_3000_3500_step1 = pd.DataFrame({'index': index_list, 'llm_3000_3500_step1': llm_response_list})

with open('llm_3000_3500_step1.pkl', 'wb') as file:
    pickle.dump(llm_3000_3500_step1, file)
...
try:
    for index, row in final_df[3500:4000].iterrows():
        free_text = row['text']
        user_message = step1_query_optimized.format(free_text=free_text)
        openai_message = create_prompt(system_message, user_message)
        response = send_message(openai_message, deployment_name)
...
llm_3500_4000_step1 = pd.DataFrame({'index': index_list, 'llm_3500_4000_step1': llm_response_list})

with open('llm_3500_4000_step1.pkl', 'wb') as file:
    pickle.dump(llm_3500_4000_step1, file)
